...
for n in grain_list:
    gb_ct = n[9]
    for m in gb_ct:
        ax_a.plot(m[:,1], m[:,0],'k',linewidth=0.5)
        ax_b.plot(m[:,1], m[:,0],'k',linewidth=0.5)
        ax_c.plot(m[:,1], m[:,0],'k',linewidth=0.5)

for n in grain_boundary_V:
    for m in n:
        ax_a.plot(m[:,1], m[:,0],'r',linewidth=0.5)
        
for n in grain_boundary_L:
    for m in n:
        ax_b.plot(m[:,1], m[:,0],'r',linewidth=0.5)
        
for n in grain_boundary_AWV:
    for m in n:
        ax_c.plot(m[:,1], m[:,0],'r',linewidth=0.5)
        
ax_a.set_title('Voronoi Tessellation')
ax_b.set_title('Laguerre Tessellation')
ax_c.set_title('AWVT')
...
xy_data = np.concatenate(xy_data)
xy_V = np.concatenate(xy_V)
xy_L = np.concatenate(xy_L)
xy_AWV = np.concatenate(xy_AWV)
...
s_AWV = []
for n in xy_data:
    q = np.sum((xy_V - n)**2, 1).min()
    s_V.append(px_size * q**0.5)
for n in xy_V:
    q = np.sum((xy_data - n)**2, 1).min()
    s_V.append(px_size * q**0.5)
    
for n in xy_data:
    q = np.sum((xy_L - n)**2, 1).min()
    s_L.append(px_size * q**0.5)
for n in xy_L:
    q = np.sum((xy_data - n)**2, 1).min()
    s_L.append(px_size * q**0.5)

for n in xy_data:
    q = np.sum((xy_AWV - n)**2, 1).min()
    s_AWV.append(px_size * q**0.5)
for n in xy_AWV:
    q = np.sum((xy_data - n)**2, 1).min()
    s_AWV.append(px_size * q**0.5)

#%%
bin_edges = np.arange(0, 45, 5)   # edges from -4 to 4 with step 0.5

plt.rcParams.update({'font.size': 16})
fig = plt.figure(figsize=(6, 4))
ax_d = plt.subplot(1,1,1)
ax_d.hist(s_V, bins=bin_edges, density=True, alpha=0.8, edgecolor='black')
ax_d.set_xlabel('$\u0394r$ (\u00B5m)', fontsize = 22)
ax_d.set_ylabel('Probability', fontsize = 22)
ax_d.set_xlim(-2, 42)
ax_d.set_ylim(0, 0.2)
fig.tight_layout()
...
plt.rcParams.update({'font.size': 16})
fig = plt.figure(figsize=(6, 4))
ax_e = plt.subplot(1,1,1)
ax_e.hist(s_L, bins=bin_edges, density=True, alpha=0.8, edgecolor='black')
ax_e.set_xlabel('$\u0394r$ (\u00B5m)', fontsize = 22)
ax_e.set_ylabel('Probability', fontsize = 22)
ax_e.set_xlim(-2, 42)
ax_e.set_ylim(0, 0.2)
fig.tight_layout()
...
ax_f.hist(s_AWV, bins=bin_edges, density=True, alpha=0.8, edgecolor='black')
ax_f.set_xlabel('$\u0394r$ (\u00B5m)', fontsize = 22)
ax_f.set_ylabel('Probability', fontsize = 22)
ax_f.set_xlim(-2, 42)
ax_f.set_ylim(0, 0.2)
fig.tight_layout()
